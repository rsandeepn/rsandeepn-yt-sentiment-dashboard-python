...
# NOUN_PATTERN = re.compile(r"\b[a-zA-Z][a-zA-Z0-9]+\b")
...
#     for text in texts:
#         tokens = NOUN_PATTERN.findall(text.lower())
...
# ----------------------------
# Lightweight keyword extraction (NO spaCy)
# ----------------------------
...
# Clustering
# ----------------------------
def cluster_and_summarize(comments, num_clusters=5):
    if not comments:
        return {}
    ...
    for i, label in enumerate(labels):
        clusters.setdefault(str(label), []).append(comments[i])

    return {
        k: {
            "summary": summarize_cluster(v),
            "examples": v[:EXAMPLE_LIMIT]
        }
...
def summarize_cluster(comments):
    if not comments:
        return "No insights"
    keywords = extract_keywords(comments, 4)
    return "Key discussion around: " + ", ".join(keywords)

# ----------------------------
# Suggestions
# ----------------------------
SUGGESTION_PHRASES = [
    "should","please","can you","could you","need more",
    "do a video","make a video","next time","improve"
]

def detect_suggestions(items):
    return [
        i for i in items
        if len(i["text"]) > 15 and any(p in i["text"].lower() for p in SUGGESTION_PHRASES)
    ]

# ----------------------------
# Theme overview
# ----------------------------
def summarize_theme_overview(clusters, label):
    if not clusters:
        return f"No major {label} themes found."
...
def analyze_comments(video_url: str):
    ...
    theme_overview = {
        "positive": summarize_theme_overview(pos_clusters, "positive"),
        "negative": summarize_theme_overview(neg_clusters, "negative"),
        "neutral": f"There are {len(neutrals)} neutral comments without strong sentiment."
    }
